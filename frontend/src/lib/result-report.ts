...
function estiloParagraph(a: QuizAnswers, estilo: number): string {
  ...
  if (est === "muito_fraco" || est === "fraco") {
    parts.push("Seu estilo ainda nao assina uma identidade clara: a leitura rapida e de 'neutro demais' ou sem direcao.");
  } else if (est === "medio") {
    parts.push("Seu estilo esta no meio-termo: funciona no dia a dia, mas nao amplifica a impressao que voce quer causar.");
  } else {
    parts.push("Seu estilo tem base solida; o foco agora e alinhar detalhes para que a mensagem visual fique coerente com seus objetivos.");
  }
  if (cuidado === "baixa") parts.push("A rotina de cuidado baixa puxa a nota de estilo para baixo mesmo quando o potencial existe.");
  if (roupa === "pratico_sem_estilo" || roupa === "aleatorio") parts.push("Roupas muito praticas ou irregulares comunicam falta de intencao — o cerebro do outro interpreta como baixa prioridade de imagem.");
...
function presencaParagraph(a: QuizAnswers, pres: number): string {
  ...
  const parts: string[] = [];
  if (post === "inseguranca") parts.push("Postura e expressao puxam para inseguranca percebida — antes de qualquer roupa cara, o corpo conta a historia.");
  else if (post === "neutralidade") parts.push("Voce transmite neutralidade: nao afasta, mas tambem nao ancora atencao.");
  else parts.push("Confianca percebida ajuda a sustentar presenca; combine com olhar e ritmo para nao parecer 'duro' ou distante.");
  if (att === "quase_nunca") parts.push("Quando a presenca nao chama atencao no ambiente, o padrao e ser lembrado pelo papel social, nao pela atracao visual.");
  else if (att === "sempre") parts.push("Voce chama atencao — o risco e a leitura errada se o visual nao acompanha a energia.");
  parts.push(`${pres.toFixed(1)}/10 na dimensao presenca.`);
  return parts.join(" ");
}

const TIP_POOL: { test: (a: QuizAnswers) => boolean; text: string }[] = [
  { test: (a) => a.q06_frequenciaCuidado === "baixa", text: "Agende 2 blocos de 20 min por semana so para grooming (cabelo/barba/pele): constancia vence intensidade esporadica." },
  { test: (a) => a.q04_estiloAtual === "fraco" || a.q04_estiloAtual === "muito_fraco", text: "Monte um mini-capsula de 5 pecas neutras que combinam entre si — reduz decisao matinal e sobe o nivel mediano do visual." },
  { test: (a) => a.q07_roupaDiaADia === "aleatorio", text: "Defina uma 'uniforme informal' para trabalho/saida: 2 calcas, 3 tops, 1 camada — mesmo gosto, menos ruido visual." },
  { test: (a) => a.q08_posturaExpressao === "inseguranca", text: "Exercicio diario: 2 minutos em pe, ombros para tras, queixo neutro, olhar ao horizonte — antes de entrar em qualquer ambiente." },
  { test: (a) => a.q09_primeiroContato === "amigavel_nao_marcante", text: "Teste uma assinatura visual leve (cor ou textura) para deixar de ser 'simpatico neutro' sem parecer fora do seu estilo." },
  { test: (a) => a.q05_aparenciaFotos === "mal" || a.q05_aparenciaFotos === "muito_mal", text: "Tire 10 fotos de rosto com luz lateral suave; escolha 1 como referencia de angulo seguro para redes e WhatsApp." },
  { test: (a) => a.q10_chamaAtencao === "quase_nunca", text: "Entrada em ambiente: 3 segundos de pausa na porta, sorriso leve, primeiro contato visual — isso aumenta presenca sem 'performar'." },
  { test: (a) => a.q11_bloqueioAtual === "fotos_redes", text: "Alinhe foto de perfil, primeira imagem do feed e foto de WhatsApp — leitura inconsistente entre canais mata confianca." },
  { test: (a) => a.q11_bloqueioAtual === "estilo", text: "Liste 3 itens que voce usa por habito mas nao gosta; remova um por semana ate sentir leveza no espelho." },
  { test: (a) => a.q11_bloqueioAtual === "confianca", text: "Combine ajuste fisico pequeno (postura + roupa) com uma micro-acao social diaria (cumprimentar com nome)." },
  { test: (a) => a.q11_bloqueioAtual === "comunicacao", text: "Treine abertura de conversa em uma linha honesta + pergunta aberta; evite monologo quando nervoso." },
  { test: (a) => a.q14_disposicaoAjustar === "total" || a.q14_disposicaoAjustar === "alta", text: "Com sua disposicao alta, aplique uma mudanca visivel na semana 1 (corte, barba, ou par de sapatos) para criar momentum." },
  { test: (a) => a.q13_tempoMelhora === "7_dias", text: "Plano de 7 dias: dia 1 espelho, dia 2 roupa, dia 3 foto, dia 4 postura, dia 5 grooming, dia 6 redes, dia 7 revisao." },
  { test: (a) => a.q03_objetivoAtual === "relacionamento_serio", text: "Para relacionamento serio, priorize sinais de cuidado e estabilidade (roupa limpa, barbear consistente, arrumacao)." },
  { test: (a) => a.q03_objetivoAtual === "poder_social", text: "Para presenca social, uma peca com estrutura (jaqueta/camisa bem ajustada) eleva autoridade percebida rapidamente." },
...
function pickPracticalTips(answers: QuizAnswers, archetypeId: string, limit: number): string[] {
  ...
  const reordered = [...TIP_POOL].sort((a, b) => stableHash(a.text + orderSeed) - stableHash(b.text + orderSeed));
...
const FIRST_IMPRESSION: Record<string, string> = {
  marcante: "Voce e lido(a) como presenca forte desde o inicio; o risco e parecer inacessivel. Suavize com sorriso sincero e primeira frase calorosa.",
  educado_neutro: "Primeira impressao: educado(a) e seguro(a), mas sem tensao romantica. Adicione um detalhe memoravel (cor, acessorio, timing de olhar) para sair do neutro.",
  amigavel_nao_marcante: "Voce e recebido(a) como simpatico(a), mas nao como opcao. A assinatura visual e o ritmo de entrada no ambiente precisam de mais intencao.",
  intimidante_distante: "Leitura inicial: distante ou dificil de ler. Abra o corpo, reduza barreiras (fone, bracos cruzados) e use primeira frase clara.",
  confuso: "Sinais mistos na primeira impressao: ajuste postura + roupa para uma mensagem unica — 'confiante e acolhedor' ou 'elegante e direto', mas nao os dois brigando.",
};
...
function sevenDayPlan(archetypeId: string, a: QuizAnswers): string[] {
  ...
  if (archetypeId === "imagem_inconsistente") base[0] = "Dia 1: checklist matinal de 7 minutos (cabelo, roupa, pele) — mesma ordem todos os dias.";
  if (archetypeId === "intensidade_desalinhada") base[1] = "Dia 2: alinhar roupa ao tom da sua energia (mais estrutura ou mais leveza, nao os dois ao mesmo tempo).";
  return base;
}

function simulation(score: ScoreResult, seed: string): { projected: number; pct: number; body: string } {
  ...
  const pillars: PillarBlock[] = [
    {
      ...
      body: answersComplete ? estiloParagraph(answers, score.estiloScore) : `Componente de estilo integrado na nota global: ${score.estiloScore.toFixed(1)}/10 (detalhamento limitado porque o questionario chegou incompleto no relatorio).`,
    },
    {
      key: "simetria",
